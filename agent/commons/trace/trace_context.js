...
if (HANDLE_RETENTION > 0) {
  global.eggLogger.info('use trace context setTimeout to handle trace data') 
} else if (HANDLE_RETENTION === 0) {
  global.eggLogger.info('use cls module to handle trace data') 
} else {
  global.eggLogger.info('HANDLE_RETENTION is invalid: ' + HANDLE_RETENTION) 
}
...
TraceContext.prototype.endTraceObject = function endTraceObject() {
  ...
  if (this.callStack.length !== 0) {
    ...
    //    global.eggLogger.debug('not matching the span event!'); 
    //}
...
TraceContext.prototype.dataSend = function dataSend() {
  try {
    ...
    global.eggLogger.info('local process send udp data failed: ' + err) 
  }
}
...
TraceContext.prototype.recordException = function recordException(err) {
  if (this.isRoot() && this.hasInit()) {
    ...
    global.eggLogger.info('cannot record err,this.spanRecord has not init!')
  }
}
...
TraceContext.prototype.resetNamespaceTransaction = function resetNamespaceTransaction(traceContext) {
  try {
    ...
    global.eggLogger.info('reset namespace transaction error' + err) 
  }    
}
...
function traceDataSend() {
  global.eggLogger.debug('start analyse trace metadata')
  /* jshint validthis: true */
  var traceContext = this.traceContext
  if (traceContext) {
    global.eggLogger.debug('current trace metadata is not undefined')
    ...
    if (TRACE_MANAGER_ENABLE) {
      try {
        ...
        if (!ret) {
          global.eggLogger.debug('return false when send data to child process')
        }
      } catch(err) {
        global.eggLogger.error('send trace data to child process failed: ' + err) 
      }
    } else {
      traceContext.dataSend() 
    }
  } else {
    global.eggLogger.debug('current trace metadata is undefined')
  }
