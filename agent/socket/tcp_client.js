...
TcpClient.prototype.connect = function (dataHandler) {
  ...
  global.eggLogger.info('use tcp server ' + this.host + ':' + this.port)
  this.client.connect(this.port, this.host, function () {
    global.eggLogger.info('build connection success')
  })

  this.client.on('data', dataHandler)

  this.client.on('close', function () {
    global.eggLogger.info('connection closed')
    this.client.end()
  }.bind({client: this.client}))

  this.client.on('error', function (err) {
    global.eggLogger.warn('connection error:' + err)
    this.client.destroy()
  }.bind({client: this.client}))
}
...
TcpClient.prototype.send = function (message, callback){
  ...
  var status = this.client.write(message, function (err) {
    if (err) {
      global.eggLogger.warn('message request failed!')
    }
    global.eggLogger.info('message request success!')
  })
