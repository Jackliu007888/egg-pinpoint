...
var interval = global.pinpointConfig.collectorInterval ||  10000
...
function agentStatMonitor() {
  ...
  pidUsage.statAsync(pid)
    ...
    .then(function (agentStat) {
      ...
      global.eggLogger.debug('send agent stat:' + JSON.stringify(agentStat))
      udpStatClient.send(agentStat)

    })
    .catch(function (err) {
      if (err) {
        global.eggLogger.error('get agent stat error: ' + err)
      }
    })
}
...
function agentStatCollector(udpStatClient, pidInfo) {
  ...
  if (TRACE_MANAGER_ENABLE) {
    if (pidInfo) {
      ...
      global.eggLogger.info('get parent process info,pid is: ' + pid + ',heap mem is: ' + heap_max_memory)
    } else {
      global.eggLogger.info('get parent process info failed,stop stat collector!')
      return
    }
  } else {
    pid = process.pid 
    ...
    global.eggLogger.info('get current process info,pid is: ' + pid + ',heap mem is: ' + heap_max_memory)
  }
