...
function startAgentStatCollector(udpStatClient, data) {
  global.eggLogger.info('start agent stat collector in child process')
...
var Client = function udpClient() {
  global.eggLogger.info('init udp client in child process')
...
process.on('message', function get(data) {
  ...
  global.eggLogger.info('receive new data, start handle it')
  global.eggLogger.debug('new data is: ' + JSON.stringify(data))
  try {
    ...
    global.eggLogger.error('child process send udp data failed: ' + err) 
  }
